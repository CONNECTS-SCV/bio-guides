...
for ch in sorted(dual):
    print(f"     '{ch}' —", " / ".join(f"{role} {sorted(set(e))}" for role, e in dual[ch].items()))
...
if not mine_p.exists():
    ...
    print("  공통 entry 중 값이 달라진 컬럼:", drift or "없음 (완전 일치)")
...
made = sorted(p.name for p in pathlib.Path("my_run").glob("demo_*.csv"))
print(f"\\n판정 — my_run 에 생긴 CSV {len(made)}개 —", ", ".join(made) or "없음")
print("      4개(imgt/chothia × H/KL)가 정상이에요. 0개면 hmmscan 이 없을 가능성이 커요(Ch.03 의 3.0).")
...
def compare(fname):
    ...
    if not mine_p.exists():
        print(f"{fname}: my_run 산출물 없음 → 대조 건너뜀"); return None
    if not ref_p.exists():
        print(f"{fname}: 레퍼런스 없음 → 대조 건너뜀"); return None
    mine, ref = pd.read_csv(mine_p), pd.read_csv(ref_p)
    common = [c for c in ref.columns if c in mine.columns]
    hard = [c for c in common if c not in SOFT]
    same = mine[hard].astype(str).equals(ref[hard].astype(str))
    print(f"{fname}: 공통 컬럼 {len(common)}개 | germline·numbering {len(hard)}개 일치 = {same}")
    if not same:
        for col in hard:
            if not mine[col].astype(str).equals(ref[col].astype(str)):
                print("   차이:", col, "| 내 결과", mine[col].tolist(), "| 레퍼런스", ref[col].tolist())
    for col in [c for c in SOFT if c in common]:
        m, rf = [str(x) for x in mine[col].tolist()], [str(x) for x in ref[col].tolist()]
        if m != rf:
            print(f"   {col} 차이 — 내 결과 {m} / 레퍼런스 {rf}")
    return same

res = [compare(f) for f in ["demo_imgt_H.csv", "demo_imgt_KL.csv",
                            "demo_chothia_H.csv", "demo_chothia_KL.csv"]]
done = [x for x in res if x is not None]
if not done:
    print("\\n대조할 my_run 산출물이 없어요 — 1절 셀을 먼저 실행하세요.")
else:
    print(f"\\n판정 — 대조한 {len(done)}개 파일 중 germline·numbering 일치 {sum(done)}개.")
    print("      germline·numbering 이 같고 bit score 만 다르면 정상이에요(ANARCI/HMM 프로파일 DB 버전 차이).")
    print("      보고서엔 도구 버전을 함께 적으세요.")'''),
...
(BioPhi 기본값 — scheme=kabat, cdr_definition=kabat, 1 iteration, CDR 은 humanize 하지 않음)'''),
          code('''run_tool([PY, SCRIPTS/"sapiens_humanize.py", "data/demo_mab.fa",
...
if len(diff):
    ...
    print("region 분포 =", diff["region"].value_counts().to_dict())
    if n_cdr == 0:
...
> `RUN_IGFOLD = False` 로 두면 예측을 건너뛰고 커밋된 PDB(`data/`)로 나머지 절을 진행해요.'''),
          code('''RUN_IGFOLD = True     # 예측 자체는 CPU 9초 실측. False 면 커밋본으로 진행
...
기준선은 본문 그래프와 같은 **1 Å** 입니다.'''),
          code('''CONF = 1.0      # 본문 6.2 그래프의 빨간 기준선 (< 1 Å 이면 신뢰)
...
for chain, v in sorted(res.items()):
    ...
    over = [(f"{chain}{n}{i}", b, region_of(chain, n)) for n, i, b in sorted(v, key=lambda r: -r[2])
            if b > CONF]
    peaks += over
    print(f"chain {chain}: {len(v)} res | mean {sum(err)/len(err):.2f} Å | "
          f"max {top:.2f} Å at {chain}{num}{ins} ({region_of(chain, num)})")
    print(f"  기준선 {CONF:g} Å 초과 {len(over)}개 → "
          + (", ".join(f"{lbl} {b:.2f} Å" for lbl, b, _ in over) or "없음"))

hot = sorted({r for _, _, r in peaks})
print(f"\\n판정 — 1 Å 을 넘는 잔기 {len(peaks)}개, 소속 구간 = {hot or ['없음']}")
...
print(f"\\n원자 {n_atoms}개 · 원자 종류 = {sorted(atom_names)}")
print("스크립트가 do_refine=False 로 돌려서 **side chain 이 없는 backbone(+CB) 모델**이에요. "
...
print("사슬별 최대 예측오차 —", {k: f"{v:.2f} Å" for k, v in sorted(flat.items())})
...
for c1, c2, out in [("H", "N", "my_run/contacts_H_N.tsv"),
                    ("H", "L", "my_run/contacts_H_L.tsv")]:
...
print(f"paratope ({ab_chain}) {len(para)}개 = {[ab_chain + r for r in para]}")
print(f"epitope  ({ag_chain}) {len(epi)}개 = {[ag_chain + r for r in epi]}")
...
print("CDR 분포 =", cdr_tab["region"].value_counts().to_dict())
if n_cdr == len(para):
    print(f"→ paratope {len(para)}개가 **전부 CDR 안**이에요. 이론대로 CDR 이 결합을 주도합니다.")
else:
...
if mine_p.exists():
    ...
    if only_mine or only_ref:
        print("내 결과에만 =", only_mine)
        print("레퍼런스에만 =", only_ref)
    print("원자접촉 수가 다른 pair =", diff_n or "없음")

    if mine == ref:
        print("→ pair 집합과 접촉 수가 **완전 일치**. 재현 성공이에요.")
    else:
...
if pathlib.Path(out2).exists():
    ...
    print(f"\\n{CUTOFF} Å → {len(hn):>2} pairs · {hn['atom_contacts'].sum():>3} atom contacts · "
          f"epitope {len(e1)}개 · paratope {len(p1)}개")
    print(f"{CUT2} Å → {len(wide):>2} pairs · {wide['atom_contacts'].sum():>3} atom contacts · "
          f"epitope {len(e2)}개 · paratope {len(p2)}개")
    print(f"새로 들어온 epitope = {sorted(e2 - e1, key=resi_key) or '없음'} · "
          f"새로 들어온 paratope = {sorted(p2 - p1, key=resi_key) or '없음'}")
    print(f"→ 0.5 Å 차이로 pair 가 {len(hn)}→{len(wide)}, atom contacts 가 "
          f"{hn['atom_contacts'].sum()}→{wide['atom_contacts'].sum()} 으로 움직여요. "
          f"epitope 목록을 인용할 때는 **cutoff 를 함께** 적어야 비교가 성립합니다.")
else:
...
print(f"주황 스틱 paratope {len(para)}개 = {[ab_chain + r for r in para]}")
print(f"빨간 스틱 epitope  {len(epi)}개 = {[ag_chain + r for r in epi]}")
...
motif 컬럼에는 **hit 위치(1-based, 세미콜론 구분)** 가 들어가고, hit 이 없으면 빈 칸입니다.'''),
          code('''run_tool([PY, SCRIPTS/"liability_scan.py", "data/demo_mab.fa",
...
if not mine_p.exists():
    ...
    print(f"행 {len(mine)} vs {len(ref)} | 공통 컬럼 {len(common)}개 | 값 완전 일치 = {same}")
    if only:
...
> 실전 benchmark 라면 여러 건강 도너의 여러 unit 을 합치세요(같은 스크립트에 `--url` 만 바꾸면 됩니다).'''),
          code('''run_tool([PY, SCRIPTS/"fetch_oas_unit.py", "--out", "my_run/oas_subset.tsv.gz"])
