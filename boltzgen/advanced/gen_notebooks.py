...
# ─────────────────────────────────────────────────────────────────────────────
# 설계 결과 3D 뷰어 — 05·07·08·09·10·11 실습 챕터 공통 셋업 코드
#   · 최종 CIF(rank*.cif)를 gemmi 로 읽어 py3Dmol 인라인 뷰어에 띄운다(Colab 그대로 동작).
#   · 설계 사슬 ↔ 타깃 사슬은 메트릭 CSV 의 설계 서열과 사슬 서열을 대조해 정한다(라벨 하드코딩 없음).
#   · find_one 이 my_run/ → 커밋된 data/ 순으로 찾으므로 어느 결과인지도 함께 출력된다.
# ─────────────────────────────────────────────────────────────────────────────
V3D = r'''import importlib.util, sys, pathlib
...
VIEW_TIP = ("마우스로 회전·확대할 수 있어요(휠 = 줌, 드래그 = 회전). "
            "구조가 안 보이면 `py3Dmol` 설치 로그를 확인하고 셀을 한 번 더 실행하세요.")
...
for ch in model:
    ...
    off = poly[0].seqid.num - poly[0].label_seq
    rows.append({
        "체인": ch.name,
        "폴리머 길이": len(poly),
        "종류": kind,
        "파일 잔기번호": f"{poly[0].seqid.num}..{poly[-1].seqid.num}",
        "명세 res_index": f"{poly[0].label_seq}..{poly[-1].label_seq}",
        "번호 차이": f"{off:+d}" if off else "같음",
        "서열 (앞·뒤)": seq if len(seq) <= 30 else seq[:14] + "…" + seq[-14:],
    })
display(pd.DataFrame(rows))
print("'번호 차이' 가 0 이 아닌 체인은 파일 번호를 그대로 명세에 적으면 엉뚱한 잔기를 가리켜요.")"""),
...
# res_index 범위 표기는 include / exclude / binding 이 전부 같은 문법을 써요.
import pandas as pd
from IPython.display import display

print("res_index 범위 표기 — include · exclude · binding 공통")
display(pd.DataFrame([
    {"표기": "45..55", "뜻": "45번부터 55번까지", "쓰는 곳": "결합부위 한 구간"},
    {"표기": "..10", "뜻": "처음부터 10번까지", "쓰는 곳": "N말단 꼬리 잘라내기"},
    {"표기": "185..", "뜻": "185번부터 끝까지", "쓰는 곳": "C말단 꼬리 잘라내기"},
    {"표기": "10,29,33,40..48", "뜻": "개별 번호와 범위를 섞어 쓰기", "쓰는 곳": "흩어진 접촉 잔기"},
]))'''),
...
display(pd.DataFrame([
    {"목적": "빠른 테스트 · 환경 검증", "권장 --num_designs": "4 ~ 100", "이 노트북": "★ 여기"},
    {"목적": "일반 설계", "권장 --num_designs": "1,000 ~ 5,000", "이 노트북": ""},
    {"목적": "어려운 타깃 · 복잡한 제약", "권장 --num_designs": "10,000 ~ 60,000", "이 노트북": ""},
]))
print("시간·계산이 비례해 늘어나요. 작게 테스트한 뒤 크게 프로덕션으로 갑니다.")

print("\\n--budget — 점수 상위 N개가 아니라, 서로 다른 전략까지 골라 담는 최종 후보 수 (본문 4.3)")
display(pd.DataFrame([
    {"프로토콜": "peptide-anything", "권장 --budget": 30},
    {"프로토콜": "protein-small_molecule", "권장 --budget": 40},
    {"프로토콜": "protein-anything", "권장 --budget": 50},
]))
print("본문 4.7 실행 예시의 값이에요. 1~3등이 거의 같은 서열이면 하나가 실패할 때 셋 다 실패하니,")
print("다양성을 섞는 게 실험 성공 확률을 높이는 보험입니다.")

print("\\n아래 실습은 첫 표의 맨 윗줄, num_designs 4 · budget 2 스모크 규모로 갑니다.")'''),

md("""## 2) 실행 중 무엇을 보며 기다리나 (본문 4.8)

다음 셀은 **5분 동안 로그만 쏟아냅니다.** 무엇을 보며 기다릴지 먼저 정해두면
"멈춘 건지 도는 건지"로 헷갈리지 않아요."""),
co('''import shutil, subprocess, pandas as pd
from IPython.display import display

print("로그에서 이 세 줄만 쫓으면 됩니다 (본문 4.8)")
display(pd.DataFrame([
    {"로그에 뜨는 줄": "Pipeline step k of N", "뜻": "k 번째 스텝 시작",
     "이럴 때 봅니다": "k 가 1 → 2 → 3 으로 오르면 정상"},
    {"로그에 뜨는 줄": "Step ... completed successfully in Xs", "뜻": "그 스텝 정상 종료",
     "이럴 때 봅니다": "Xs 로 어느 스텝이 무거운지 감을 잡아요"},
    {"로그에 뜨는 줄": "Traceback", "뜻": "예외로 중단",
     "이럴 때 봅니다": "위로 스크롤해 원인 확인"},
]))
print("같은 k 에서 오래 머무르면 멈춘 게 아니라 그 스텝이 무거운 거예요.")

print("\\n터미널에서 돌릴 때 (본문 4.8)")
display(pd.DataFrame([
    {"명령": "watch -n 1 nvidia-smi", "무엇을 보나": "다른 터미널에서 GPU 사용량"},
    {"명령": "boltzgen run ... > run.log 2>&1 &", "무엇을 보나": "백그라운드로 돌리고 로그를 파일로"},
    {"명령": "tail -f run.log", "무엇을 보나": "그 로그를 실시간 추적"},
]))
...
print(f"이 실행에서 실제로 돌아간 스텝 {len(names)}개")
display(pd.DataFrame([
    {"순서": i, "스텝": nm,
     "하는 일": WHAT.get(nm, ("-", "-"))[0],
     "결과가 쌓이는 곳": WHAT.get(nm, ("-", "-"))[1]}
...
print("본문 4.6 트리 대조 — 있어야 할 폴더가 실제로 생겼나")
rows = []
for rel, note in EXPECT:
    p = OUT_ROOT / rel.rstrip("/")
    if p.is_dir():
        rows.append({"있음": "O", "경로": rel, "크기": f"{sum(1 for _ in p.glob('*'))}개", "무엇": note})
    elif p.is_file():
        rows.append({"있음": "O", "경로": rel, "크기": f"{p.stat().st_size}B", "무엇": note})
    else:
        rows.append({"있음": "-", "경로": rel, "크기": "-", "무엇": note})
display(pd.DataFrame(rows))
...
print("실제로 들어 있는 것 (하위 폴더별 파일 수·확장자)")
display(pd.DataFrame([
    {"폴더": key + "/", "파일 수": f"{len(seen[key])}개",
     "확장자": ", ".join(f"{e} {n}" for e, n in Counter(seen[key]).most_common(4))}
...
if agg is not None:
    ...
    print(f"analysis 산출 {agg.shape[1]}컬럼 → filtering 이 {len(added)}개를 덧붙였어요. 무엇이 붙었나 —")
    crows = []
    for kind, pre in [("하드필터 통과 여부", "pass_"), ("메트릭별 순위", "rank_"), ("종합 점수·순위", "")]:
        ...
        if got:
            crows.append({"분류": kind, "개수": f"{len(got)}개",
                          "컬럼 (앞 4개)": " · ".join(got[:4])
                                        + (f" … 외 {len(got) - 4}개" if len(got) > 4 else "")})
    display(pd.DataFrame(crows))
...
if not RANKS:
    ...
    print("\\n각 디자인의 발목을 잡은 지표 — 여섯 순위 중 가장 나빴던 것")
    display(pd.DataFrame([
        {"최종 순위": int(rk), "디자인": wid, "발목 잡은 지표": w,
         "그 지표의 순위": int(s.loc[s["id"] == wid, "rank_" + w].iloc[0])}
...
inside = set(df["id"])
print(f"\\n이 셋이 최종셋(budget {len(df)}) 에 들었나")
display(pd.DataFrame([
    {"디자인": r["id"], "최종셋": "안" if r["id"] in inside else "밖 — ipTM 은 높은데 밀렸어요",
     "final_rank": int(r["final_rank"]) if "final_rank" in r else "-"}
...
md(f"""## 10) 만든 구조를 눈으로 — rank 1 디자인 3D (본문 5.10)

여기까지는 전부 숫자였어요. 마지막은 **실제로 무엇을 만들었는지**를 봅니다.
`final_ranked_designs/` 의 rank 1 CIF 에는 서열뿐 아니라 좌표가 들어 있으니 그대로 띄우면 돼요.

**주황이 설계 단백질, 파랑이 타깃**입니다. {VIEW_TIP}"""),
co(V3D + '''
top = df.sort_values("final_rank").iloc[0]
print("rank 1 디자인 :", top["id"])
pdb, model, DESIGN, TARGET, MATCH = load_design(
    f"final*designs/*{top['id']}*.cif", "data/vanilla", row=top)
...
print("\\n무엇을 봐야 하나")
print(f"  · 주황(사슬 {','.join(DESIGN)}) 설계 단백질이 파랑 타깃(사슬 {','.join(TARGET)}) "
      "표면에 맞물려 붙어 있으면 정상이에요.")
print("  · 설계 사슬이 타깃에서 떨어져 따로 떠 있거나, 실뭉치처럼 풀려 있으면 실패 신호예요 —")
print("    3)의 pTM·ipTM 과 4)의 그래프를 같이 보고 후보에서 빼세요.")
if {"design_ptm", "design_to_target_iptm", "filter_rmsd"} <= set(df.columns):
    print(f"\\n이 디자인의 숫자 — pTM {top['design_ptm']:.3f} / ipTM {top['design_to_target_iptm']:.3f}"
          f" / RMSD {top['filter_rmsd']:.2f} A (기준 0.7 / 0.5 / 2.0)")'''),
...
display(flip.reset_index(drop=True).round(5))"""),
...
display(pd.DataFrame([
    {"필터": name,
     "거르는 잔기": name.split("_")[1],
     "탈락": f"{int((~df[name].astype(bool)).sum())}개",
     "통과": f"{int(df[name].astype(bool).sum())}개"}
    for name in comp]))
...
c += boot("07_peptide_cyclic", pip="pandas matplotlib gemmi py3Dmol")
...
co("""import itertools, pandas as pd
from IPython.display import display
...
print(f"SG 원자 {len(sg)}개 · 이상적인 SG-SG 거리는 2.0~2.1 A 예요")
drows = []
for i, j, dist in bonded + [p for p in pairs if p[2] >= 2.5][:2]:
    step = abs(j - i)
    gap  = min(step - 1, L - step - 1)          # 고리 기준 최단 간격(사이에 낀 잔기 수)
    drows.append({"Cys 쌍": f"CYS{i}-CYS{j}", "SG-SG 거리 (Å)": round(dist, 2),
                  "판정": "이황화" if dist < 2.5 else "결합 아님",
                  "사이 잔기": gap if dist < 2.5 else "-",
                  "서열 간격": ("권장 3~15 안" if 3 <= gap <= 15 else "권장 범위 밖")
                              if dist < 2.5 else "-"})
...
md(f"""## 9) 고리와 매듭을 눈으로 — rank 1 디자인 3D (본문 7.3·7.4)
...
md("""## 10) 더 좁히고 싶을 때 — 2차구조와 결합부위 (본문 7.6·7.7)
...
print("성격이 다른 셋을 함께 보내는 게 정석이에요 — 어떤 축에서 뽑힌 후보인지 함께 봅니다.")
display(pd.DataFrame([
    {"뽑힌 이유": label, "최종 순위": int(d.loc[idx, "final_rank"]), "디자인": str(d.loc[idx, "id"]),
     "ipTM": round(float(d.loc[idx, "design_to_target_iptm"]), 3),
     "pTM": round(float(d.loc[idx, "design_ptm"]), 3),
     "RMSD (Å)": round(float(d.loc[idx, "filter_rmsd"]), 2)}
...
c += boot("08_antibody_fab", pip="pandas matplotlib gemmi py3Dmol")
...
for _, r in df.head(5).iterrows():
    ...
    row = {"최종 순위": int(r["final_rank"]), "디자인": r["id"]}
    if has_nd:
        row["num_design (명세)"] = int(r["num_design"])
    row["사슬별 재설계"] = "+".join(str(len(p)) for p in parts)
    row["합계"] = total
    if has_nd:
        row["명세와 일치"] = "O" if total == int(r["num_design"]) else "X 새어 나감"
    row["재설계 구간 Cys"] = sum(p.count("C") for p in parts)
    rows.append(row)
display(pd.DataFrame(rows))
...
if "liability_violations_summary" in df.columns:
    ...
    _s = sum(tot.values()) or 1
    display(pd.DataFrame([{"모티프": name, "검출": f"{n}건", "비중": f"{100*n/_s:.0f}%"}
...
display(pd.DataFrame([
    {"최종 순위": int(r["final_rank"]), "디자인": r["id"],
     "사슬 종류": LABEL.get(chain_kind(str(r[col])), chain_kind(str(r[col]))),
     "길이 (aa)": len(str(r[col])), "Cys": str(r[col]).count("C"),
     "서열 (앞·뒤)": f"{str(r[col])[:6]}… …{str(r[col])[-11:]}"}
    for _, r in df.head(5).iterrows() for col in chain_cols]))
...
display(pd.DataFrame([{"사슬 종류": LABEL.get(k, k), "개수": f"{v}개"}
...
for label, col, asc in axes:
    ...
    row = {"축": label}
    for k, (i, v) in enumerate(zip(top3["id"], top3[col]), 1):
        row[f"{k}위"] = f"{i} ({v:.3f})" if isinstance(v, float) else f"{i} ({v})"
    arows.append(row)
print("축마다 상위 3개를 뽑아 보면 — 세 줄에 같이 나오는 이름이 균형 잡힌 후보예요.")
display(pd.DataFrame(arows))
...
md(f"""## 9) 만든 Fab 을 눈으로 — rank 1 디자인 3D (본문 8.4·8.6)
...
결과 CSV 의 `designed_sequence_*` 를 `full_sequence_*` 위에 되짚어 찾습니다. {VIEW_TIP}"""),
co(V3D + '''
...
for cid in DESIGN:
    ...
    print(f"  사슬 {cid} ({des_col}) — CDR {len(segs)}구간 "
          + " · ".join(f"{a}~{b}" for a, b in segs) + f" = {len(resi)}잔기 (분홍)")
...
c += boot("09_nanobody", pip="pandas matplotlib gemmi py3Dmol")
...
for _, r in df.iterrows():
    ...
    row = {"최종 순위": int(r["final_rank"]), "디자인": r["id"], "전체 길이 (aa)": len(s)}
    if has_nd:
        row["num_design (CDR1+2+3)"] = int(r["num_design"])
    row["서열에서 센 CDR3"] = f"{len(cdr3)} aa"
    row["CDR3 서열"] = cdr3
    crows.append(row)
print("num_design 과 '서열에서 센 CDR3 길이' 는 서로 다른 수예요 — 두 열을 나란히 놓고 봅니다.")
display(pd.DataFrame(crows))
...
for _, r in df.iterrows():
    ...
    cys_ok += s.count("C") == 2
    trows.append({"최종 순위": int(r["final_rank"]), "디자인": r["id"], "Cys": s.count("C"),
                  "N말단": head, "N말단 판정": "일치" if not dn else "치환 " + " · ".join(dn),
                  "C말단": tail, "C말단 판정": "일치" if not dc else "치환 " + " · ".join(dc)})
...
md(f"""## 9) 만든 나노바디를 눈으로 — rank 1 디자인 3D (본문 9.3·9.5)

5)절에서 CDR3 를 서열로 세어 봤죠. 그 CDR3 가 타깃 쪽으로 뻗어 있는지는 구조로만 보입니다.

**주황이 설계 VHH, 파랑이 타깃 penguinpox 단백질**이고
**분홍이 재설계 CDR1·CDR2, 초록이 CDR3**(마지막 구간)예요. 구간은 CSV 의 `designed_sequence` 를
`designed_chain_sequence` 위에 되짚어 찾습니다 — 번호를 적어 두지 않아요. {VIEW_TIP}"""),
co(V3D + '''
top = df.iloc[0]                                  # df 는 이미 final_rank 순으로 정렬돼 있어요
print("rank 1 디자인 :", top["id"])
pdb, model, DESIGN, TARGET, MATCH = load_design(
    f"final*designs/*{top['id']}*.cif", "data/nanobody", row=top)
...
for cid in DESIGN:
    ...
    for k, seg in enumerate(segs, 1):
        last  = (k == len(segs))                  # 마지막 재설계 구간 = CDR3
        color = "#27ae60" if last else "#e2247f"
        resi  = seg_resi(model, cid, [seg])
        view.addStyle({"chain": cid, "resi": resi}, {"cartoon": {"color": color}})
        view.addStyle({"chain": cid, "resi": resi},
                      {"stick": {"colorscheme": "greenCarbon" if last else "magentaCarbon",
                                 "radius": 0.18}})
        cdrrows.append({"사슬": cid, "CDR": f"CDR{k}", "잔기 범위": f"{seg[0]}~{seg[1]}",
                        "길이 (aa)": len(resi), "그림 색": "초록 — CDR3" if last else "분홍"})

if cdrrows:
    display(pd.DataFrame(cdrrows))
view.zoomTo()
view.show()

print("\\n무엇을 봐야 하나")
print("  · 초록 CDR3 루프가 파랑 타깃 표면으로 뻗어 파고들어 있으면 정상이에요 — VHH 결합의 대부분을 CDR3 가 맡습니다.")
print("  · 주황 VHH 가 타깃에서 떨어져 있거나 CDR 이 반대쪽을 보고 있으면 ipTM 이 낮은 이유가 그거예요.")
print("  · framework(주황 β-barrel) 가 통짜로 유지돼 있으면 7)절의 N·C말단 검증과 그림이 일치하는 겁니다.")
try:
    print("\\n5)절이 서열에서 센 CDR3 —", cdr3_of(str(top[SEQ])))
except Exception:
    pass'''),
...
c += boot("10_small_molecule", pip="pandas matplotlib gemmi py3Dmol")
...
md("""## 9) 이 챕터에서 확인한 것 (본문 10.6~10.7)
...
c += boot("11_nucleic_acid", pip="pandas matplotlib gemmi py3Dmol")
...
for l in lines:
    for key, ko in KIND.items():
        if key in l and l.split()[1] == key:
            seq = l.split()[-1]
            rows.append({"종류": ko, "길이": f"{len(seq)} 잔기",
                         "서열": seq[:44] + ("…" if len(seq) > 44 else "")})
zn = [l for l in lines if l.startswith("HETATM") and l.split()[5] == "ZN"]
rows.append({"종류": "Zn 이온", "길이": f"{len(zn)} 개",
             "서열": "비폴리머라 원자 단위로 셉니다"})
display(pd.DataFrame(rows))'''),
...
display(pd.DataFrame([
    {"finger": i, "Cys 2자리": f"{site[0]} · {site[1]}", "His 2자리": f"{site[2]} · {site[3]}",
     "not_design 고정": "O 전부 고정" if all(r in NOT_DESIGN for r in site) else "X 새어 나감"}
...
if "designed_chain_sequence" in dna.columns:
    print("설계 서열의 Cys 개수 (6 finger x 2 = 12 이면 그대로 보존)")
    display(pd.DataFrame([
        {"최종 순위": int(r["final_rank"]), "디자인": r["id"],
         "길이 (aa)": len(str(r["designed_chain_sequence"])),
         "Cys": str(r["designed_chain_sequence"]).count("C"),
         "판정": "보존" if str(r["designed_chain_sequence"]).count("C") == 12
                 else ("부족 — not_design 범위 확인" if str(r["designed_chain_sequence"]).count("C") < 12
                       else "자유 Cys 생김"),
         "His": str(r["designed_chain_sequence"]).count("H")}
...
print("ipTM 높은 순 — 상한이 어느 디자인에서 나왔는지 RMSD 와 같이 보세요")
display(sub.reset_index(drop=True))
...
md(f"""## 9) 두 복합체를 눈으로 — DNA·RNA rank 1 디자인 3D (본문 11.5·11.6)

H-bond 30개와 9개의 차이가 어디서 오는지는 그림 한 장이면 끝나요.
먼저 DNA 쪽입니다 — **주황이 설계 zinc finger 단백질, 청록이 DNA 두 가닥, 보라 구가 Zn²⁺**,
그리고 **노란 stick 이 Zn 을 실제로 붙잡고 있는 잔기**(거리 3Å 안, 4)절의 그 자리)예요. {VIEW_TIP}"""),
co(V3D + '''
...
md(f"""이번엔 RNA 쪽이에요. 같은 색 규칙(**주황 = 설계 단백질, 청록 = RNA**)으로 띄우면
타깃 크기 차이가 그대로 보입니다 — 36+36 nt 이중나선 vs 20 nt 헤어핀. {VIEW_TIP}"""),
co('''rna_top = rna.sort_values("final_rank").iloc[0]
