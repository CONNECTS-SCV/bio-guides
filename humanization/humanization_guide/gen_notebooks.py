...
항체 전용 언어모델 `mogam-ai/Ab-RoBERTa` 로 각 position 을 차례로 `<mask>` 로 가리고, **그 자리에 있던 진짜 잔기의 로그확률**을 모아 평균해요(masked pseudo-log-likelihood). 지표는 네 가지예요.

- `mean_log_prob` — **높을수록** 자연스러움
...
짝은 잔기 번호가 아니라 **서열 정렬로** 지어요. 우리 VL 은 humanization 뒤에도 길이가 111 로 같은데 중간이 한 칸 밀려 있어서, 번호로 짝지으면 서로 다른 잔기끼리 재게 되거든요.
...
def pair_index(a, b):
    """두 서열을 정렬해 0-based 잔기 인덱스를 1:1 로 짝지어요 — indel 자리는 짝 없이 빠져요."""
    m = {}
...
4. 6개 CDR 중 가장 크게 움직인 건 **L-CDR3 (1.9455 Å)** 인데 변이는 **1개**뿐이에요. 반대로 변이가 4개인 L-CDR1 은 0.1857 Å. **변이 개수로 구조 변화를 예측할 수 없어요.**
5. 짝은 잔기 번호가 아니라 **서열 정렬**로 지어요 — VL 은 길이가 111 로 같은데도 **109 자리만** 대응돼요(짝 없는 자리가 parental·humanized 양쪽 2곳씩).
