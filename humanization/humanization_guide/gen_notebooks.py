...
co('''from IPython.display import display
...
try:
    ...
    display(pd.DataFrame([
        {"체인": "VH", "길이 (aa)": len(VH), "chain_type": ch_h.chain_type,
         "무엇으로 붙었나": "중쇄", "CDR3": ch_h.cdr3_seq, "CDR3 길이": len(ch_h.cdr3_seq)},
        {"체인": "VL", "길이 (aa)": len(VL), "chain_type": ch_l.chain_type,
         "무엇으로 붙었나": {"K": "경쇄 kappa", "L": "경쇄 lambda"}.get(ch_l.chain_type, "경쇄"),
         "CDR3": ch_l.cdr3_seq, "CDR3 길이": len(ch_l.cdr3_seq)},
    ]))
...
SEQ_OF = {"H": VH, "L": VL}
PICKS  = {"H": (5, 12, 78, 115), "L": (31, 85, 109, 111)}

rows = []
for chain in ("H", "L"):
    for k in PICKS[chain]:
        if k not in r2i[chain]:
            continue
        imgt = r2i[chain][k]
        rows.append({"체인": "VH" if chain == "H" else "VL",
                     "raw 번호": k,
                     "잔기": SEQ_OF[chain][k - 1] if k <= len(SEQ_OF[chain]) else "?",
                     "IMGT 라벨": imgt,
                     "비고": "IMGT 범위 밖 — tail 로 라벨링" if "tail" in imgt else ""})
last_l = max(r2i["L"])
rows.append({"체인": "VL", "raw 번호": last_l,
             "잔기": VL[last_l - 1] if last_l <= len(VL) else "?",
             "IMGT 라벨": r2i["L"][last_l], "비고": "VL 마지막 잔기"})

print("두 번호 체계가 어떻게 어긋나는지 — 같은 잔기를 raw 와 IMGT 로 각각 부른 것")
display(pd.DataFrame(rows))
print(f"판정 — Sapiens 가 'I78T' 라고 하면 raw 78번이고, 같은 잔기의 IMGT 번호는 {r2i['H'].get(78)} 이에요.")
print("       Ch.06 의 도구 간 합의는 이 변환을 거쳐야 성립해요.")'''),
...
print(f"이번 절이 비교할 후보 {len(cands)}종 — 길이가 parental 과 다르면 indel 이 생긴 거예요.")
display(pd.DataFrame([
    {"후보": k,
     "VH 길이 (aa)": len(cands[k][0]),
     "VL 길이 (aa)": len(cands[k][1]),
     "parental 대비": "기준" if k == "parental" else
                     ("길이 같음" if (len(cands[k][0]), len(cands[k][1])) == (len(VH), len(VL))
                      else f"VH {len(cands[k][0])-len(VH):+d} · VL {len(cands[k][1])-len(VL):+d} (indel)"),
     "어디서 왔나": "내 결과" if SRC[k].startswith("내 결과") else
                   ("러닝 예제" if k == "parental" else "레퍼런스 (앞 챕터 my_run 이 없어 커밋본 사용)"),
     "파일": SRC[k].split(" · ")[-1]}
    for k in cands]))
...
print("\\npaired 기준 순위 — mean_logp 가 0 에 가까울수록 자연스러워요.")
display(pd.DataFrame([{"순위": i, "후보": r.variant,
                       "mean_logp": round(r.mean_logp, 4),
                       "perplexity": round(r.perplexity, 4),
                       "잔기 수": int(r.n_residues)}
...
for n in order:
    r = float(ppl[n]) / base
    v_ = "통과" if r <= 1.0 else ("주의" if r < 2.0 else "경고 — 서열 무너짐 의심")
    if r >= 2.0:
        warn.append(n)
    cut.append({"후보": n, "perplexity": round(float(ppl[n]), 4),
                "parental 대비": f"{r:.2f}배", "판정": v_,
                "막대": "█" * max(1, round(r * 6))})
display(pd.DataFrame(cut))
...
for col, lab in (("overall_score", "overall"), ("FR_score", "FR"), ("CDR3_score", "CDR-H3")):
    ...
    deltas[lab] = b - a
    drows.append({"구간": lab, "parental": round(a, 4), ref_cand: round(b, 4),
                  "Δ": f"{b - a:+.4f}"})
...
try:
    ...
    # torch 2.6 부터 torch.load 의 weights_only 기본값이 True 로 바뀌어 IgFold 체크포인트가
    # UnpicklingError 로 죽어요. IgFold 가 배포하는(신뢰된) 가중치라 False 로 되돌립니다.
    _orig_load = torch.load
    torch.load = lambda *a, **k: _orig_load(*a, **{**k, "weights_only": False})

    print("IgFold 모델 4개 + AntiBERTy 로딩 중… (첫 실행은 가중치 다운로드 포함)")
    _quiet = io.StringIO()                    # JHU 라이선스 안내와 ckpt 경로 나열을 삼켜요
    with contextlib.redirect_stdout(_quiet):
        from igfold import IgFoldRunner
        t0 = time.time(); runner = IgFoldRunner(); load_s = time.time() - t0
    print(f"로딩 완료 {load_s:.1f}초")
    ...
    print(f"IgFold 실행 실패 — {type(e).__name__}")
    print("  " + " ".join(str(e).split())[:300])
...
display(pd.DataFrame([
    {"구조": n,
     "레퍼런스 실측 (초)": round(ref_t[n], 2),
     "내 폴딩 (초)": round(mine[n], 1) if n in mine else "미실행",
     "이 랩이 쓸 파일": str(MY / f"{n}.pdb") if n in mine else str(REF / f"{n}.pdb")}
    for n in targets]))
if not mine:
    print("내 폴딩 열이 전부 '미실행' 이면 위의 실패 사유를 먼저 보세요 — 수치 자체는 레퍼런스로 그대로 나옵니다.")'''),
co('''import pandas as pd
...
for n, t in tabs.items():
    ...
    for chain in ("H", "L"):
        ...
        prm.append({"구조": n, "체인": f"V{chain}",
                    "전체 (Å)": round(float(s[col].mean()), 3),
                    "CDR (Å)": round(float(cdr_m), 3),
                    "framework (Å)": round(float(fr_m), 3),
                    "CDR / framework": f"{cdr_m / fr_m:.1f}배"})
display(pd.DataFrame(prm))
...
co('''print("위 그림에서 무엇을 보고 있는지 — 숫자 세 개만 붙잡으면 됩니다.")
display(pd.DataFrame([
    {"구간": "VH framework", "RMSD (Å)": round(fwv, 4),
     "그림에서 보이는 것": "두 cartoon 이 사실상 포개짐", "읽는 법": "골격은 그대로 = 기준선"},
    {"구간": "CDR-H3", "RMSD (Å)": round(h3v, 4),
     "그림에서 보이는 것": "굵게 칠한 loop 만 벌어짐",
     "읽는 법": f"framework 의 {h3v/fwv:.1f}배 — 벌어지는 곳은 여기뿐"},
    {"구간": "VH 전체", "RMSD (Å)": round(whv, 4),
     "그림에서 보이는 것": "-",
     "읽는 법": "loop 의 변화가 나머지 잔기에 희석된 값"},
]))
print(f"CDR-H3 에 들어간 변이 {len(mut_list)}개 — " + (" · ".join(mut_list) or "없음"))
print("판정 — 곁사슬은 바뀌었는데 backbone cartoon 은 거의 같은 자리예요.")
print("       그래서 '변이가 있다 = 구조가 망가졌다' 가 아닙니다.")
if HAVE_3D:
...
print(f"\\nCh.10 으로 넘어가는 값은 CDR-H3 RMSD {h3v:.4f} Å 하나뿐이에요 — 나머지 표는 그 값을 해석하기 위한 맥락이에요.")
...
# unpaired Cys — 개수가 홀수면 짝 없는 시스테인이 남아요. 출처와 한 표에 놓습니다.
odd, rows = [], []
for name, (h, l) in cands.items():
    nh, nl = h.count("C"), l.count("C")
    bad = bool(nh % 2 or nl % 2)
    if bad:
        odd.append(name)
    rows.append({"후보": name, "Cys VH": nh, "Cys VL": nl,
                 "짝": "홀수 — unpaired Cys" if bad else "짝 맞음",
                 "출처": "내 결과" if SRC[name].startswith("내 결과") else
                        ("러닝 예제" if name == "parental" else "레퍼런스"),
                 "파일": SRC[name].split(" · ")[-1]})
display(pd.DataFrame(rows))
print(f"unpaired Cys 판정 — 홀수 후보 {len(odd)}개"
...
print("\\n후보별 총량과 그중 oxidation 이 차지하는 몫")
display(pd.DataFrame([
    {"후보": name, "liability 총량": f"{int(tot[name])}건",
     "oxidation": f"{int(ox[name])}건",
     "oxidation 비중": f"{ox[name]/tot[name]*100:.0f}%",
     "나머지 모티프": f"{int(tot[name]) - int(ox[name])}건"}
    for name in cands]))
...
print("\\nparental 에 없던 모티프가 새로 생긴 개수 (parental 은 기준이라 제외)")
display(pd.DataFrame([{"후보": name, "신규 모티프": f"{int(v_)}건"}
...
print(f"대조한 행 {len(cmp_)}개 · 개수 " + ("일치" if ok_cnt else "어긋남")
      + " · 위치 " + ("일치" if ok_pos else "어긋남"))
...
print("\\n레퍼런스 기준 후보별 총량과, parental 에 없던 N-glyc 가 새로 생겼는지")
display(pd.DataFrame([
    {"후보": name, "liability 총량": f"{int(ref_tot[name])}건",
     "parental 대비": "기준" if name == "parental"
                     else f"{int(ref_tot[name]) - int(ref_tot['parental']):+d}건",
     "신규 N-glyc": "★ 생김" if name in ref_new_glyc else "없음"}
    for name in cands]))
