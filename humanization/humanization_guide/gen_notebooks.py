...
- **로컬**: 이 노트북을 `{chapter}/` 폴더 안에서 열었다면 클론 없이 그대로 진행돼요.
...
각 자리에서 확률이 가장 높은 잔기를 고르면(argmax) 그게 곧 Sapiens-humanized 서열이에요.

**가드 없이** 돌려요 — 본문이 경고한 사고를 직접 재현하기 위해서예요."""),
...
# 첫 실행은 HuggingFace 에서 Sapiens 가중치(~2MB)를 받아요. HF 가 느리면 실패할 수 있어
# 두 번까지 재시도하고, 그래도 안 되면 레퍼런스(data/)로 넘어가요 — 실습은 끊기지 않아요.
...
if not HAVE_SAPIENS:
    print("[레퍼런스] data/ 의 Sapiens 실행 산출물로 진행해요")
...
Ch.04 에서 못 박은 CDR 보호 좌표(`cdr_guard.json`)를 가져와, 방금 나온 mutation 중 **CDR 안에 떨어진 것**을 찾아요.
Ch.04 를 건너뛰었다면 이 챕터 `data/cdr_table_imgt.csv` 로 폴백해요."""),
...
if gp.exists():
    ...
    print("[레퍼런스] data/cdr_table_imgt.csv 에서 CDR 좌표를 복원해요")
...
print("VL CDR1 =", "".join(VL[p-1] for p in sorted(guard["L"])[:9]),
      "→ 가드 없는 argmax 가 CDR-L1 을 통째로 갈아엎었어요.")
print("이건 결합력을 깰 수 있는 위험한 mutation 입니다 — 그래서 CDR 좌표를 미리 못 박아야 합니다.")'''),

md("""## 3) 직접 실행 — CDR 가드 적용본 만들기 (본문 5.3 주의)

실무 처방은 셋 중 하나예요 — ① CDR 을 mask 에서 제외, ② 도구의 CDR 보호 모드, ③ **후처리로 CDR 을 parental 로 되돌리기**.
여기서는 ③ 을 그대로 구현해요(가장 단순하고 검증하기 쉬움)."""),
...
print("→", MY / "sapiens_humanized_cdrguard.fasta", "(Ch.08 구조 검증에서 이 파일을 쓸 수 있어요)")'''),

md("""## 4) 직접 실행 — humanness, **정의를 못 박고** 계산 (본문 5.4)

여기가 본문에서 가장 헷갈리는 지점이에요. "humanized humanness" 는 두 가지로 계산될 수 있고, **값이 달라요.**
...
**본문 표의 0.815 / 0.872 는 (b) 입니다.** (a) 로 계산하면 0.782 / 0.851 이 나와요 — 둘 다 직접 계산해 확인해요."""),
co('''if HAVE_SAPIENS:
    ...
    hn = pd.DataFrame({
        ...
        "(b) CDR 가드 적용본": [float("nan")] * len(piv),      # 가드 적용본은 직접 돌려야 나와요
    })

display(hn)
print("본문 표(0.694→0.815 / 0.770→0.872)는 parental → **(b)** 열이에요.")
print("(a) 로 계산하면 0.782 / 0.851 — 같은 실행, 다른 정의, 다른 값이에요.")
print("CDR 가드를 적용하면 humanness 는 조금 내려가요 — CDR 을 사람화하지 않았으니 당연하고, 그게 맞아요.")'''),

md("""## 5) 그래프 — parental vs humanized (공용 모듈)

`humanization_viz.humanness_bars` 로 체인별 비교를 그려요."""),
...
`data/` 는 실제 실행 산출물이에요. mutation 리스트가 **문자 단위로 같은지**, humanness 가 같은지 봅니다."""),
...
2. **가드 없이 돌리면 CDR-L1 이 부서져요**(`H31A K32Y F33N P34D`) — Ch.04 의 보호 좌표로 후처리 복원하면 CDR mutation 0개.
3. humanness 는 **정의를 밝혀야** 합니다. 본문의 **0.694 → 0.815 (VH)** · **0.770 → 0.872 (VL)** 는 **(b) humanized 재스코어링 self-prob**. 같은 실행에서 (a) 로 계산하면 0.782 / 0.851 로 **달라요**.
4. 가드 적용본은 humanness 가 조금 낮아요 — CDR 을 그대로 뒀으니 당연하며, 결합력을 지키는 대가예요.
...
md("""## 6) 내 결과 확인 — 본문의 "78번이 유일한 3도구 합의" 는 사실이 아니에요
...
항체 전용 언어모델 `mogam-ai/Ab-RoBERTa` 로 각 position 을 차례로 마스킹해 **실제 잔기의 로그확률**을 모아 평균해요(masked pseudo-LL).
...
후보 서열은 **앞 랩에서 내가 만든 것**을 먼저 씁니다(Ch.05 Sapiens · Ch.06 Humatch/AnthroAb). 없으면 `data/variants.fasta` 로 폴백해요.
...
try:
    ...
    print("→ 레퍼런스(data/abroberta_scores_summary.csv)로 이어가요.")
    abr = pd.read_csv("data/abroberta_scores_summary.csv")'''),

md("""## 2) 내 결과 확인 — naturalness ≠ humanness (본문 7.2.2)

`mean_log_prob` 는 **높을수록**(0 에 가까울수록) 자연스럽고, `perplexity = exp(-mean_logp)` 는 **낮을수록** 좋아요.

여기서 본문이 짚은 반직관적 사실을 직접 봅니다 — **VH 만 보면 parental 이 가장 자연스러워요.**
...
print("→ 사람다움(Sapiens humanness·Humatch CNN·AbNatiV)이 올라간 후보가 "
      "naturalness 에서는 오히려 내려갈 수 있어요. Ab-RoBERTa 는 주 humanness 지표로 쓰면 안 됩니다.")'''),
co('''from humanization_viz import humanness_bars
import numpy as np
...
print("VH 막대를 보세요 — humanized 가 parental 보다 낮아요(사람다워졌지만 덜 '자연스러워짐').")
...
**스코어링 자체는 4서열에 3.5~6.6초로 끝나요.** 무거운 건 **체크포인트**입니다 —
`abnativ init` 은 9개 ckpt(~6GB)를 **순차로** 받아 매우 느려요(실측: 30분에 3개). 실제로 필요한 4개만 **병렬로** 받으면 **약 33분 / 2.6GB**(실측)입니다.

그래서 아래 셀은 **`RUN_ABNATIV = False` 가 기본**입니다. 그대로 두면 커밋된 `data/abnativ_summary_all_models.csv`(실제 실행 산출물)로 이어져요.
직접 돌리려면 `True` 로 바꾸세요."""),
co('''RUN_ABNATIV = False        # ← True 로 바꾸면 체크포인트를 받아 실제로 스코어링해요

abn_csv = None
if RUN_ABNATIV:
    try:
        _ensure("abnativ anarci")
        # abnativ init 은 9개 ckpt 를 순차 다운로드해 매우 느려요 → 필요한 4개만 병렬로(실측 33분)
    ...
    print("RUN_ABNATIV = False → 커밋된 레퍼런스(data/abnativ_summary_all_models.csv)로 진행해요.")
    print("  · 스코어링은 빠르지만(3.5~6.6초) 체크포인트 다운로드가 약 33분 / 2.6GB 입니다(실측).")'''),

md("""## 4) 내 결과 / 레퍼런스 — nativeness 프로파일 (본문 7.1.3)

AbNatiV 에는 **두 세대**가 있고, 본문의 두 표는 사실 **서로 다른 모델**입니다 — 이걸 모르면 숫자가 모순돼 보여요.

- **AbNatiV1**(`-nat VH` / `-nat VLambda`) — 본문 7.1.3 의 주 표(0.648 → 0.880 · FR 0.632 → 0.925 · CDR-H3 0.614 → 0.626 · VL 0.902). **7개 숫자 전부 재현**됩니다.
- **AbNatiV2**(`-nat VH2` / `-nat VL2`) — 본문 7.2.3 패널의 "AbNatiV H" 열. parental **0.4927 은 재현**되지만, humanized 의 **0.6900 은 재현되지 않아요** — 실측은 **0.7777**.

그리고 본문의 "VL 0.902" 는 **parental** VL 입니다(humanized VL 은 0.9980). 이것도 표에서 확인해요."""),
...
print("\\nAbNatiV2 VH2 humanized 실측:",
      round(float(_v2.overall_score.iloc[0]), 4),   # .iloc[0] — pandas 2.x 는 float(Series) 를 거부해요
      "(본문의 0.6900 은 재현되지 않아요)")'''),
...
세 지표는 **서로 다른 것**을 잽니다. 한 줄로 놓고 보면 어긋나는 지점이 바로 보여요."""),
...
display(panel)
print("읽는 법 — nativeness 는 Sapiens 후보가 가장 높지만(0.8803), naturalness 로 보면 순위가 달라져요.")
print("권장 역할 분담: 주 패널 = Sapiens humanness + Humatch CNN + AbNatiV, "
      "Ab-RoBERTa = naturalness 이상치 탐지 보조.")'''),

md("""## 이 랩에서 확인한 것

1. **AbNatiV 는 두 세대**입니다. 본문 7.1.3 표는 **AbNatiV1** — VH **0.6477 → 0.8803**, FR **0.6317 → 0.9245**, CDR-H3 **0.6137 → 0.6265**(거의 불변), parental VL **0.9022**. 7개 값 모두 재현돼요. 본문의 "VL 0.902" 는 **parental** 값이에요(humanized VL 은 0.9980).
2. **AbNatiV2** 는 절반만 재현돼요 — parental VH2 **0.4927**(일치), humanized VH2 **0.7777**(본문 0.6900 과 불일치).
3. **Ab-RoBERTa** — parental **−0.7240** · Humatch **−0.7717** 는 본문과 정확히 일치하지만, Sapiens **−0.4973**(본문 −0.6928)·AnthroAb **−0.5285**(본문 −0.8733)는 **재현되지 않아요.** 실측값을 씁니다.
4. **naturalness ≠ humanness** — VH 만 보면 parental(−0.5188)이 가장 자연스러워요.
...
서열 지표가 좋아져도 **CDR loop 모양이 망가지면 결합력이 떨어져요.** 그래서 만든 서열을 **독립된 모델에게 다시 접어보게** 합니다.

IgFold(+AntiBERTy)로 **VH+VL Fv 하나를 7~12초**에 접어요(실측: parental 7.1초 · humanized 12.0초).
실행 시 반드시 아래 두 옵션을 끕니다 — 실제로 부딪힌 함정이에요.
...
try:
    ...
    print("→ 아래 분석은 레퍼런스 구조(data/parental.pdb · data/sapiens_humanized.pdb)로 이어져요.")'''),

md("""## 2) 내 결과 확인 — per-residue 신뢰도 (B-factor = 예측 RMSD)

IgFold 는 PDB 의 B-factor 자리에 **per-residue 예측 오차(Å)** 를 적어요 — **낮을수록 확신**입니다.
CDR 구간(특히 CDR-H3)에서 값이 튀는 게 정상이에요."""),
...
print("CDR 구간(분홍)에서 예측 오차가 커지는 게 정상이에요 — loop 라서.")
for n, t in tabs.items():
...
(전체를 한꺼번에 정렬하면 loop 의 변화가 framework 오차에 묻혀요.)"""),
...
display(res)
print("CDR-H3 RMSD 가 framework RMSD 보다 크면 → humanization 이 loop 를 흔들었다는 뜻이에요.")'''),
...
print("       Sapiens 가 CDR-H3 안에 mutation 을 넣지 않았는데도 loop 가 조금 움직인 건, "
      "framework 치환이 loop 받침대(Vernier)를 통해 간접적으로 영향을 주기 때문이에요.")'''),

md("""## 이 랩에서 확인한 것

1. IgFold 로 **VH+VL Fv 를 7~12초**에 접었어요 — `do_refine=False`(PyRosetta 없음) · `do_renum=False`(VL C-말단 잔기가 IMGT 범위 밖) 가 필수.
2. per-residue 신뢰도(B-factor)는 **CDR 에서 커져요** — loop 라서 정상.
3. **framework 로 정렬한 뒤 잰 CDR-H3 RMSD = 0.5406 Å**(framework 자체는 0.2707 Å). CDR-H3 에 mutation 이 없는데도 loop 가 움직인 것은 framework 치환의 간접 효과예요.
4. 이 값이 랭킹(Ch.10)의 **구조 보존 항목**으로 들어가요.
...
핵심은 **parental 대비 증분** — humanization 이 **새로 만든** liability 를 잡는 것이에요."""),
...
print("\\n읽는 법 — 신규 N-glyc 모티프가 CDR/paratope 근처에 생기면 hard filter 대상이에요(Ch.10).")'''),
...
같은 모티프라도 **CDR/paratope 안**이면 위험도가 달라요(결합에 직접 영향).
"""),
...
print("주의 — CDR 좌표는 parental 기준이에요. indel 이 있는 후보(Humatch VL)는 위치가 한 칸씩 밀릴 수 있어요.")'''),
...
1. liability 스캔은 **정규식 4줄**이면 끝나요 — 후보를 만들 때마다 자동으로 돌리세요.
2. 중요한 건 절대 개수가 아니라 **parental 대비 신규 모티프**입니다(특히 신규 `NXS/T`).
3. CDR 안에 떨어진 모티프는 위험도가 달라요 — Ch.10 의 **hard filter** 로 연결돼요.
